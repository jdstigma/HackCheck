...
| 5 | **Signed release build** | User wants a proper signed copy (keystore + release build config) once the current app state is confirmed working. Directly useful now even ahead of Play Store publication (#1) — a signed APK installs/updates cleanly outside Android Studio and is a prerequisite for #1 regardless. Blocked on: user field-testing the Cell Tower Locator `requestCellInfoUpdate()` fix (commit `a74f3cf`, fixed 2026-08-04 — drove away from home and the screen kept showing only the home tower; switched off the documented-stale `allCellInfo` cache to a forced fresh scan) to confirm it actually tracks towers away from home before calling the app state "confirmed." | 💡 proposed — waiting on field test |
